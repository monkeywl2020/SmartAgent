...
def _try_loading_included_file_formats() -> (
    ...
    try:
        from llama_index.readers.file import (
            ...
            PandasExcelReader,
            PDFReader,
            wlPDFReader, # 修改为自定义的 wlPDFReader
            PptxReader,
            VideoAudioReader,
        )  # pants: no-infer-dep
    except ImportError:
    ...
    default_file_reader_cls: dict[str, Type[BaseReader]] = {
        ".hwp": HWPReader,
        #".pdf": PDFReader,  去掉
        ".pdf": wlPDFReader,# 修改为自定义的 wlPDFReader
        ".docx": DocxReader,
        ".pptx": PptxReader,
        ".ppt": PptxReader,
        ".pptm": PptxReader,
...
# 自定义的 wlSimplePdfDirectoryReader 继承自SimpleDirectoryReader
class wlSimplePdfDirectoryReader(SimpleDirectoryReader):
    #output_dir: str
    def __init__(
        self,
        input_dir: Path | str | None = None,
        input_files: list | None = None,
        output_dir: str = ".", # 默认是使用当前目录。工程运行的当前目录
        savePdfToPng: bool = False, # 是否将pdf文件截图保存为png,默认不保存
        page_overlap: int = 200, # 页重叠部分，默认200 字符，汉字，英文都是字符
        excluded_content: list[tuple[str, str]] = None, # 在Document中需要排除掉的内容,字符串或者正则表达式
        exclude: list | None = None,
        exclude_hidden: bool = True,
        errors: str = "ignore",
        recursive: bool = False,
        ...
        #保存输出目录，后面作为输出 pdf的截图使用
        self.output_dir = output_dir

        # 是否将pdf的每一页保存为png的截图
        self.savePdfToPng = savePdfToPng

        #页重叠部分大小默认200 字符
        self.page_overlap = page_overlap

        #在Document中需要排除掉的内容,字符串或者正则表达式，匹配到后会使用这个将文本清洗一遍
        self.excluded_content = excluded_content
    ...
    def load_pdf_file(
        ...
        savePdfToPng: bool,
        page_overlap: int,
        excluded_content: list[tuple[str, str]],
        file_metadata: Callable[[str], dict],
        file_extractor: dict[str, BaseReader],
        filename_as_id: bool = False,
        encoding: str = "utf-8",
        ...
        if file_suffix in default_file_reader_suffix or file_suffix in file_extractor:
            ...
            # load data -- catch all errors except for ImportError
            try:
                # save_png 参数ture表示要保存png图片 ,output_dir 是文件保存路径
                kwargs = {"extra_info": metadata, "outPut_file":output_dir, 
                          "save_png": savePdfToPng, "page_overlap":page_overlap,"excluded_content":excluded_content}
                if fs and not is_default_fs(fs):
                    kwargs["fs"] = fs
                docs = reader.load_data(input_file, **kwargs)
            except ImportError as e:
    ...
    def load_pdf_data(
        ...
        # 如果在 load_data的时候携带了 persist_root路径，那么就使用这个路径
        if persist_root is not None:
            persist_png_dir = os.path.join(persist_root, self.output_dir)
        else:
            persist_png_dir = self.output_dir
        ...
        if num_workers and num_workers > 1:
            ...
            with multiprocessing.get_context("spawn").Pool(num_workers) as p:
                results = p.starmap(
                    wlSimplePdfDirectoryReader.load_pdf_file, #需要处理的文件列表。
                    zip(
                        files_to_process,
                        repeat(persist_png_dir),# 将某些固定参数（如元数据、编码等）重复 N 次（N 为文件数量），使每个文件都带有这些固定参数进行处理。
                        repeat(self.savePdfToPng), # 新增参数 是否保存截图
                        repeat(self.page_overlap), # 新增参数 页之间重叠大小
                        repeat(self.excluded_content), # 新增参数 文本中去除的内容
                        repeat(self.file_metadata),
                        repeat(self.file_extractor),
                        repeat(self.filename_as_id),
                        repeat(self.encoding),
            ...
            for input_file in files_to_process:
                print(f"===============>>>{input_file}")
                # 将 解析出来的  Document 添加到 documents 列表中去
                documents.extend(
                    wlSimplePdfDirectoryReader.load_pdf_file(
                        input_file=input_file,
                        output_dir = persist_png_dir, # 新增参数 输出路径
                        savePdfToPng = self.savePdfToPng, # 新增参数 是否保存截图
                        page_overlap = self.page_overlap, # 新增参数 页之间重叠大小
                        excluded_content = self.excluded_content, # 新增参数 文本中去除的内容
                        file_metadata=self.file_metadata,
                        file_extractor=self.file_extractor,
                        filename_as_id=self.filename_as_id,
                        encoding=self.encoding,
    ...
    def load_data(
        ...
        logger.info(f"===>wlSimplePdfDirectoryReader::load_data({persist_root})")
        abc = self.load_pdf_data(persist_root = persist_root,
