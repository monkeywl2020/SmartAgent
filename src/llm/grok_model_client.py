...
#-----------------------------------
@register_llm('grok')
class GrokChatModelClient(BaseChatModelClient):
    # llm 客户端适配
    def __init__(self, 
                 config_list: Optional[Dict[str, Any]] = None, 
                 **kwargs: Any):
        logger.info("GrokChatModelClient-----------__init__")
        if config_list:
        ...
        logger.info(f"GrokChatModelClient-----------__init__  api_key:{self.api_key} base_url:{self.base_url}")
        #初始化openAI 客户端
    ...
    def generate_rsp(
        ...
        #   调用 OpenAI 接口
        #-------------------------------------------------------------
        logger.info(f"GrokChatModelClient::genrate_rsp:-------cccc---- openAIkwargs:\n{openAIkwargs}")
        response = self.client.chat.completions.create(**openAIkwargs)
        logger.info(f"GrokChatModelClient::genrate_rsp:-------cccc---- response:\n{response}")

        if stream:
            # 返回流式 迭代器
            abc = self.parse_stream_response(response)
            logger.info(f"GrokChatModelClient::genrate_rsp:-------parse_stream_response -- response:\n{response}")
            return abc
        else:
            # 返回 Message 消息
    ...
    async def a_generate_rsp(
        ...
        **kwargs: Any,
        ...
        logger.info("GrokChatModelClient::a_generate_rsp --- begin!")
        ...
        logger.info(f"GrokChatModelClient::a_generate_rsp: openAIkwargs:\n{openAIkwargs}")
        # 记录调用开始时间
        start_time = time.perf_counter()
        try:
            response = await self.async_client.chat.completions.create(**openAIkwargs)
            #response = self.client.chat.completions.create(**openAIkwargs)
            logger.info(f"GrokChatModelClient::a_generate_rsp: response: {response}")
    ...
    async def parse_stream_response_async(self, chunks: Any, start_time: float) -> AsyncIterator[List[Message]]:
        logger.info("GrokChatModelClient::parse_stream_response_async --- begin!")
        ...
        async for chunk in chunks:
            try:
                ...
                logger.info(f"GrokChatModelClient rsp:==>{chunk}")
                
                # 记录第一个有效报文的时间
                if not first_packet_logged:
                    end_time = time.perf_counter()
                    elapsed_time = end_time - start_time
                    logger.info(f"GrokChatModelClient =========>Time to first packet: {elapsed_time:.3f} seconds")
        ...
        if tool_calls:
            ...
            logger.info(f"GrokChatModelClient::parse_stream_response_async ---fff----tool_calls: {tool_calls} ")
    ...
    def parse_stream_response(self, chunks: Any) -> Iterator[List[Message]]:
        logger.info("GrokChatModelClient::parse_stream_response --- begin!")
        """解析 OpenAI 非流式响应的结果"""
        tool_calls = []  # 存储多个工具调用
    ...
    def parse_non_stream_response(self, chunk: Any) -> List[Message]:
        logger.info("GrokChatModelClient::parse_non_stream_response --- begin!")
        """解析 OpenAI 非流式响应的结果"""
        try:
            #转成dict
            chunk = chunk.model_dump()
