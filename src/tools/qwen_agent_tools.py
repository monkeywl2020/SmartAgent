...
####################################################################
#   qwen-aget 使用的 tool格式 和 openAI格式不一样，需要独立的整理
#       qwen的tool call 是配合qwen大模型使用的，调用的是qwen-agent的功能
#       
####################################################################
# 注册工具到 nuwa agent系统中去
@register_tool('recommended_service')
class recommended_service(BaseTool):
    description = '推荐心理服务。'
    parameters = [{
        'name': 'recommend_type',
        'type': 'string',
...
# 注册工具到 nuwa agent系统中去
@register_tool('switch_to_emotional')
class switch_to_emotional(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        # 验证推荐类型是否有效
        # 下面是模拟函数被调用后的结果
        switch_cmd = {'switch_mode_to':'agent_abc'}
        switch_cmd_str = json.dumps(switch_cmd)
        # qwen的 call只能返回 str
        return switch_cmd_str
    
# 注册工具到 nuwa agent系统中去
@register_tool('switch_to_taskaction')
class switch_to_taskaction(BaseTool):
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        # 验证推荐类型是否有效
        # 下面是模拟函数被调用后的结果
        switch_cmd = {'switch_mode_to':'agent_wl'}
        switch_cmd_str = json.dumps(switch_cmd)
        # qwen的 call只能返回 str
        return switch_cmd_str