...
from ..knowledge.knowledge_interface import nuwa_local_rag

'''
情商模型，专门负责情商问答，纯问答模型，基本不含tool call
'''

# 情感聊天 EmotionalChatAgent 继承自 BaseAgent
@register_agent('emotional_agent')
class EmotionalChatAgent(BaseAgent):
    ...
    async def a_generate_rsp(
        ...
        logger.info("EmotionalChatAgent::a_generate_rsp begin!")
        # 先将  self.tools_list["abc","cdd","ddfe"] 对应的工具获取出来,里面是工具名字
        emotional_tools = list(self.function_map.values())
        logger.info(f"EmotionalChatAgent::a_generate_rsp ---- emotional_tools:{emotional_tools}!")

        # 将tool转成 模型需要的格式 
        tools = self.llm_client.convert_nuwa_tool_to_model_support(emotional_tools)
        logger.info(f"EmotionalChatAgent::a_generate_rsp ---- new format tools:{tools}!")
        ...
        if 0: # 控制是否添加知识库，暂时不加
            ...
            user_input = user_message.get("content",'') # 最后一个消息是 用户消息
            # 获取rag 知识库信息。
            start_time = time.perf_counter()
            knowledge_content = nuwa_local_rag.nuwa_getRagknowledge(user_input,similarity_top_k = 1)
            end_time = time.perf_counter()
            elapsed_time = end_time - start_time
            logger.info(f"EmotionalChatAgent =========>nuwa_getRagknowledge 耗费: {elapsed_time:.3f} seconds")
        ...
        emotional_prompt = self.sys_prompt.format(knowledge = knowledge_content) + history_text
        #emotional_prompt = self.sys_prompt + history_text
        logger.info(f"EmotionalChatAgent::a_generate_rsp  ---- emotional_prompt:{emotional_prompt}!")
